...
当我们运行上面的示例时，资源监视器显示，一个名为 `Java` 的进程的 CPU 利用率已经飙升到 91.4%。
...
第一部分用于匹配 `http` 和 `https` 协议。 第二部分用于匹配 `www.` 字符。第三部分用于匹配剩余字符。我盯着正则表达式看了很长时间，没有发现什么大问题。
...
事实上，Java 用来处理正则表达式所用的 [`NFA`](https://en.wikipedia.org/wiki/Nondeterministic_finite_automaton) 引擎是引起高 CPU 利用率的关键。当进行字符匹配时， NFA 会使用一种称为“回朔法”（backtracking）的方法。一旦发生回溯，所花费的时间将变得非常长。可能是几分钟甚至几小时。时间的长短取决于发生回溯的次数和回溯的复杂度。
...
这是两种不同的自动机。在这里，我们不会深入讨论它们的原理。简单地说，`DFA` 的时间复杂度是线性的，它更稳定，但功能有限。`NFA` 的时间复杂度相对不稳定。 所以，根据正则表达式的不同，时间有时长，有时短。`NFA` 的优点是它的功能更强大。所以被 Java、.NET、Perl、Python、Ruby 和 PHP 用来处理正则表达式。

How does the `NFA` match? We use the following characters and expressions as examples.

`NFA` 怎样进行匹配？我们用下面的字符串和表达式作为例子。
...
记住，`NFA` 匹配是基于正则表达式的。也就是说，`NFA` 将依次读取正则表达式的每一个字符，并将其与目标字符串匹配。如果匹配成功，它将转到正则表达式的下一个字符。否则，它将继续与目标字符串的下一个字符进行比较。
...
*   首先，提取正则表达式的第一个匹配符：`d`。然后，将它与字符串的第一个字符 `T` 进行比较。不匹配，所以转到下一个。第二个字符是 `o`，也不匹配。那么转到下一个，也就是 `d`。匹配。于是，读取规正则表达式的第二个字符： `a`。
...
*   正则表达式的第二个匹配符是：`a`。它会与字符串的第四个字符 `a` 进行比较。又匹配了。于是继续读取正则表达式的第三个字符 `y`。
...
*   正则表达式的第三个匹配符是 `y`。让我们继续与字符串的第五个字符比较。它匹配了。接着，尝试读取正则表达式的下一个字符，发现没有字符了，因此匹配结束。
...
*   回溯是如何进行的？回溯后，字符串中已被读取的第四个字符 `c` 将被放弃。指针将返回到字符串的第三个字符。接着，正则表达式的下一个匹配符 `c` 会被用来待匹配字符串当前指针的下一个字符 `c` 进行对比。两者是匹配的。这时，字符串最后一个字符已经被读取。匹配结束。
...
另一个问题出现在正则表达式的第三部分。可以看到，有问题的URL具有下划线（`_`）和百分号（‘％’），但是与第三部分对应的正则表达式则没有。因此，只有在匹配完一长串字符之后，程序才发现两者不匹配，然后进行回溯。
...
正则表达式的第一个字符 `a` 与字符串的第一个字符 `a` 相匹配。正则表达式的第二个运算符 `b{1,3}?` 匹配了字符串的第二个字符 `b` 。由于最小匹配的原则，正则表达式的第三个运算符 `c` 与字符串第三个字符第三个字符 `b` 不匹配。因此，程序进行回溯并将正则表达式的第二个运算符 `b{1,3}?` 与字符串的第三个字符 `b` 进行比较。现在匹配成功了。之后，正则表达式的第三个匹配符 `c` 与字符串的第四个字符 `c` 正相匹配。匹配结束。
...
最后，我推荐一个网站，它可以检查你写的正则表达式以及相应的字符串匹配是否存在问题
...
本文中的正则表达式在110000次尝试之后自动停止。结果表明，正则表达式存在一定的问题并需要改进。
...
一个小小的正则表达式能让 CPU 卡死，真是太神奇了。这给了我们一个提醒。当遇到正则表达式的的时候，一定要注意“贪婪模式”以及回溯问题。
