...
当实现业务实例时，对资源进行处理是很常见的。在这个背景下，资源（例如文件或 socket 句柄）封装在对象中，使用它们后必须关闭才能释放资源。传统上开发人员有责任关闭他们所创建的资源，以防依赖冲突，一般都会写在 finally 语句块中。虽不这样做不会产生编译错误，但是很容易导致资源泄露。即使现在足够聪明的静态代码检查工具可以提供提示，但不是每个人都使用它们，而且很容易忽略这些告警。
...
Java 7 中首次引入一种新的处理（关闭）资源方式 try-with-resources。它使得在 try-catch 语句块中的资源能按照正确顺序自动关闭，可以更加便捷地处理资源。
...
让我们看一个业务实例的实现，其需要从数据库中获取指定账户的状态码。首先看在传统方式中如何实现，接着再看足智多谋的 try-with-resources 如何实现。最后，也可以看到在 Java 9 中引入的更加简洁版本如何实现。
...
如上所示，不得不增加 finally 语句块来处理资源关闭。在调用 close 方法之前，必须显示地检查null 值，并且也要保证关闭资源的逻辑顺序。这样代码不但变得冗长，而且遇到过许多开发人员会忘记编写 finally 语句块来关闭资源，导致资源泄露的情况。
...
顺便提一下，假如 try 和 finally 语句块都抛出异常，finally 语句块抛出的会抑制另一个。
...
在例子中可以看到简洁的代码有助于提高整体可读性，资源管理在这里也自动完成。try-with-resources 语句中可以包含多个资源，同时应该通过分号隔开。资源会在保持逻辑顺序的前提下自动关闭（最后声明的将第一个关闭）。
...
如果在 try-with-resources 和 try 语句块中抛出异常，从 try 中抛出的将会阻止另外一个。假如有需要，可通过调用 Throwable.getSuppressed 方法从 try 语句块抛出的异常中找回阻止的异常。
...
try-with-resources 语句中也可以编写 catch 和 finally 语句块。任何 catch 和 finally 语句块会在声明的资源关闭后运行。
...
Java 9 中引入了更加简洁的版本。如果已经把资源声明为 final 或 effective final，则在 try-with-resources 中无需创建任何新的变量，可直接使用。这样使得能够利用自动资源管理。现使用更简洁的 try-with-resources 语句来实现与上面相同的代码块，如下所示：
...
Java 7 引入了专门设计用于 try-with-resources 语句的 AutoCloseable 接口。Java 5 引入的 Closeable 接口也修改为继承 AutoCloseable 接口。这两个接口都拥有抽象的 close 方法，资源应实现并提供有效的方法。任何实现 AutoCLoseable 和 Closeable 接口的资源都可以用 try-with-resources 来关闭。所有基于 JDK 资源的类和接口都已修改成继承两个接口其中之一，使其能与现有的 try-with-resources 语句兼容。
...
try-with-resources 有助于自动资源管理，无需编写显示的 finally 语句块来处理关闭资源。以下是对 try-with-resources 要点的总结：
...
- 在 Java 7/8 ，try-with-resources 语句中必须声明要关闭的资源。这种方式声明的资源属于隐式 final。
...
- 可以让自定义的资源实现 AutoCloseable 或 Closeable 接口，以便能够在 try-with-resources 语句中使用。
